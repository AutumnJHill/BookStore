...
<!-- 넘버랑 유저이름이 비교된다(생각중~) -->
<!-- 
<%if(loginUser.getUserName() == b.getBoardWriter() ||
loginUser.getUserNo() == 1){ %>
<a href="<%=request.getContextPath() %>/InquiryUpdateForm.bd?bno=<%=b.getBoardNo() %>" class="on">수정</a>
<a href="<%=request.getContextPath() %>/InquiryDelete.bd?bno=<%=b.getBoardNo()%>">삭제</a>
<%} %>
-->